...
export const registerUser = async (req, res) => {
    try {
        const { name, email, password, isAdmin, role, title, adminPassword } = req.body;
...
export const markNotificationsList = async (req, res) => {
    try {
        const { userId } = req.user;
...
export const deleteUserProfile = async (req, res) => {
    try {
        ...
        res.status(200)
            .json({ staus: true, message: "User deleted successfully" });
    } catch (error) {
        console.log(error);
        return res.status(400).json({ status: false, message: error.message });
    }
